...
const getZodiac = () => {
  return new Promise((resolve,reject) => {
    fs.readFile(path.join(__dirname, 'horoscope.json'), 'utf8',(err,content)=>{
      if(err) {
        reject(err);
      } else {
        resolve(content);
...
function writeJSON(data) {
  fs.writeFileSync(path.join(__dirname, 'horoscope.json'), JSON.stringify(data));
  return
}
